...
const CONTAINER_INIT_DELAY_MS   = 5 * 1000; // wait after --start before opening earnapp link
...
async function loginWithCookies(page, cookies) {
  ...
  await page.goto('https://earnapp.com', { waitUntil: 'networkidle2', timeout: 30000 });
...
async function verifyNodeOnDashboard(page, earnappLink) {
  ...
  try {
    ...
    return { success: false, reason: 'Earnapp link page: confirmation element not found', ip: 'unknown' };
  }

  const alreadyLinked = await page.evaluate(
    () => document.body.innerText.includes('already linked'),
  );
  console.log(alreadyLinked ? 'Note: device was already linked.' : 'Device linked successfully.');
  ...
  try {
    ...
    return { success: false, reason: 'Devices table not found on passive-income page', ip: 'unknown' };
  }
  ...
  if (!uuidMatch) {
    return { success: false, reason: 'Cannot parse UUID from earnapp link', ip: 'unknown' };
  }
  ...
  while (!targetRow) {
    ...
    if (!targetRow) {
      if (Date.now() - findStart >= FIND_TIMEOUT_MS) {
        return { success: false, reason: `Device "${deviceName}" not found after ${FIND_TIMEOUT_MS / 1000}s`, ip: 'unknown' };
      }
      console.log(`Device not visible yet, retrying in ${FIND_RETRY_MS / 1000}s...`);
  ...
  if (banTitleEl) {
    ...
    if (banText.includes('not earning')) {
      ...
      const ip   = ipEl ? await ipEl.evaluate(el => el.textContent.trim()) : 'unknown';
      return { success: false, reason: 'Device is not earning — proxy blocked', ip };
    }
  }
  ...
  let ip = 'unknown';
  try {
    await page.waitForSelector('li.ead_stat .ead_ips p', { timeout: 5000 });
...
async function deleteNodeOnDashboard(page, earnappLink) {
  ...
  const uuidMatch = earnappLink.match(/sdk-node-([a-f0-9]+)$/i);
  if (!uuidMatch) throw new Error('Cannot parse UUID from earnapp link for deletion');
  const deviceName = `sdk-node-${uuidMatch[1].slice(-8)}`;
  ...
  if (!targetRow || !(await targetRow.evaluate(el => el && el.tagName === 'TR'))) {
    console.warn(`deleteNodeOnDashboard: device "${deviceName}" not found — skipping`);
    return;
  }
  ...
  const triggerBtn = await targetRow.$('td.eadt_actions_td button.ea_context_menu_trigger');
  if (!triggerBtn) throw new Error('Context menu trigger button not found in row');
  ...
  if (!deleteMenuBtn) throw new Error('"Delete" option not found in context menu');
  ...
  await page.evaluate(() => {
    const btn = [...document.querySelectorAll('button')].find(
      b => b.textContent.trim().includes('Delete permanently'),
    );
    if (btn) btn.click();
...
async function main() {
  ...
  for (let i = 0; i < proxies.length; i++) {
    ...
    try {
      ...
      startNode();
      console.log(`Waiting ${CONTAINER_INIT_DELAY_MS / 1000}s for container to initialize...`);
      ...
      if (verifyResult.ip && verifyResult.ip !== 'unknown') ip = verifyResult.ip;
      ...
      try { stopNode(); } catch (e) { console.warn('stopNode error:', e.message); }
